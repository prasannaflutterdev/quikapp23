...
# Enhanced logging
log() { echo "[$(date +'%Y-%m-%d %H:%M:%S')] [TEST_CERT] $1"; }
...
validate_certificate_file() {
    ...
    if [ "$file_type" = "Certificate" ]; then
        if openssl x509 -in "$file_path" -text -noout >/dev/null 2>&1; then
            log_success "Certificate file is valid"
            
            # Extract certificate information
            local subject=$(openssl x509 -in "$file_path" -noout -subject 2>/dev/null | sed 's/subject=//')
            local issuer=$(openssl x509 -in "$file_path" -noout -issuer 2>/dev/null | sed 's/issuer=//')
            local not_after=$(openssl x509 -in "$file_path" -noout -enddate 2>/dev/null | sed 's/notAfter=//')
...
# Main execution
main() {
    log_info "🧪 Starting Certificate Generation Test"
    log "================================================"
