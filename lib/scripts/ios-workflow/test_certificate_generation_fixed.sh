#!/bin/bash
# 🧪 Test Certificate Generation Script (Fixed)
# Downloads CER and KEY files and generates P12 with validation
# Handles both DER and PEM certificate formats

set -euo pipefail

# Enhanced logging
log() { echo "[$(date +'%Y-%m-%d %H:%M:%S')] [TEST_CERT_FIXED] $1"; }
...
generate_p12() {
    ...
    # Generate p12 file based on certificate format
    if [ "$cert_format" = "DER" ]; then
        log_info "Using DER certificate format for P12 generation..."
        if openssl pkcs12 -export \
            ...
            log_success "P12 file generated successfully from DER certificate"
            return 0
        else
            log_error "Failed to generate P12 file from DER certificate"
            return 1
        fi
    elif [ "$cert_format" = "PEM" ]; then
        log_info "Using PEM certificate format for P12 generation..."
        if openssl pkcs12 -export \
            -in "$cer_file" \
            -inkey "$key_file" \
            -out "$p12_file" \
            ...
            log_success "P12 file generated successfully from PEM certificate"
            return 0
        else
            log_error "Failed to generate P12 file from PEM certificate"
            return 1
        fi
    else
        log_error "Unknown certificate format, cannot generate P12"
        return 1
    fi
}
...
# Main execution
main() {
    log_info "🧪 Starting Certificate Generation Test (Fixed)"
    log "================================================"
